...
the user, catching prompt injection, PII and secret leakage, and
dangerous agent tool calls. It's wire‑compatible with the OpenAI,
...
and the scan API for batch jobs and agent tool calls.
...
| `X-Antidote-App-Id`            | App UUID (see [Apps](/runtime-security/apps)) | Required on every scan / proxy call. Attributes traffic to an App. |

---
title: 'Reverse-Proxy'
description: 'Wire-compatible passthrough endpoints for OpenAI, Anthropic, Gemini, Vertex, Bedrock, and Ollama — drop Antidote in front of LLM traffic by swapping one config value.'
---

# Runtime Security Reverse-Proxy

Wire-compatible passthrough endpoints for all supported LLM providers
that let client applications drop Antidote Runtime Security in front of
...
- Third-party tools (LangChain, LlamaIndex, raw `curl`, Cursor, etc.)
  where the wire protocol is the only contract Antidote can rely on.
...
> antidote credential as `X-API-Key` (free for OpenAI) and Anthropic
> callers send it as `X-Antidote-Key` (free for Anthropic).
...
using `X-Antidote-Upstream-Base`. The deployment must allowlist the
...
1. `X-API-Key: ak_...` — the normal antidote API key header. OpenAI
   clients should use this.
2. `X-Antidote-Key: ak_... | Bearer <jwt>` — fallback for clients
   that already use `x-api-key` for the upstream provider (i.e.
   Anthropic).
3. `antidote_session` cookie — lets the in-app dashboard exercise the
...
Every proxy call must also carry `X-Antidote-App-Id` (the UUID of an
...
3. Verifies `X-Antidote-App-Token` against the App's signed token
...
5. **Forward upstream** via `httpx.AsyncClient.post()`. Headers are
   filtered by `_filter_inbound_headers()` — hop-by-hop and antidote
...
and the aggregated latency. `_fold()` in
`runtime_security/proxy.py` implements this.
...
originated by Antidote adopt the antidote-shaped error envelope.
...
See §9 of the client guide (`runtime-security-guide.md`) for the full
schema split.
...
3. **No per-route rate limiting beyond the existing scan endpoints.**
   ...
   surface. Per-tenant upstream quota enforcement is out of scope here
   — the upstream provider is the source of truth for that.
...
- `injection_score` / `injection_label`, `pii_count`, `pii_categories`,
  `text_length`, `latency_ms`, `blocked_reason` as usual.
...
independently of the main Antidote API (and fail independently of the
...
2. **Right-sized scaling.** The proxy is latency-sensitive and runs on
   the critical path of every LLM call. The main API is not. You want
   to scale them on different signals (QPS/latency for the proxy;
   dataset-count and worker-queue depth for the main API).
...
`runtime_security_events` table that the Antidote dashboard reads, so
...
- `GET /readyz` — calls `warmup()` on the injection model and returns
  ...
  running on heuristics only. Use this for readiness probes so the
  first request doesn't pay the cold-start tax.
...
- Model load is the slow step. Keep the container warm — aggressive
  scale-to-zero will trade every cold start for ~5-15s on a fresh CPU
  pod depending on the model.
- Pin CPU limits high enough that DeBERTa inference doesn't starve on
  a shared host. Typical cost: ~10-50 ms per prompt at 512 tokens on
  modern x86-64.
...
- **Timeouts.** Default `RUNTIME_SECURITY_PROXY_TIMEOUT=60` seconds is
  chosen to match typical chat-completion latency on large models.

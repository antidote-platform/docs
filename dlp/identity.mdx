...
- **SCIM** to provision users into the Antidote workspace from your
  IdP.
...
keeps the Antidote workspace's user list in sync with your directory.
When you hire someone, they appear in Antidote automatically. When
you off‑board them, they're disabled.

### Set it up

<Steps>
  <Step title="Add a SCIM provider in Antidote">
...
Antidote roles. Common pattern:

| IdP group              | Antidote role | Reason                                            |
| ---------------------- | ------------- | ------------------------------------------------- |
...
OIDC is how employees sign into the Antidote dashboard and how the
DLP agent binds a device to a directory user.

### Configure your provider

<Steps>
  <Step title="Register Antidote as an OIDC client">
  ...
  <Step title="Add the provider in Antidote">
  ...
  <Step title="Test the flow">
    Click **Test sign-in** on the provider card. Antidote opens the
...
2. Antidote returns a short verification code and opens the IdP sign‑in
